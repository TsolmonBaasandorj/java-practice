...
public class Book {
    ...
    if(word.isEmpty()==false) {
    	if(word.contains(" ")) {
    		...
    		for(int i=0; i<arr.length; i++) {
    			if(isTitleWord(arr[i])==false && i==0 || isTitleWord(arr[i])) {
    ...
    public static Boolean isTitleWord(String word) {
		String a="a,an,the,and,but,for,nor,or,so,yet,at,by,in,into,near,of,on,to,than,via";
